...
def TriT2_sigmoidrnd(weight, train, bias, numbers):
...
def last_control(H, y_train):
    # 创建一个(L*3+n)*(L*3+n)的单位矩阵
    identity = np.identity(L*3+n)
    Ht = np.transpose(H)
...
def out_put(w, H, y_train):
    Hp = np.repeat(np.sqrt(w), 3*L+n, axis=1) * H
    Yp = np.sqrt(w) * y_train
    Hpt = np.transpose(Hp)
    return Hp, Hpt, Yp


def TriT2_RVFL(x_train, x_test, y_train, y_test):
    weight = all_weight()
    ...
    # print("*"*50)
    h = TriT2_sigmoidrnd(weight, x_train, bias1, train_nums)
    # print("H", h.shape)
    w = last_control(h, y_train)
    H, Ht, y_train1 = out_put(w, h, y_train)
    # 创建一个L*L的单位矩阵
    identity = np.identity(3*L+n)
    # 对H重赋值，保证其列满秩
    beta = np.linalg.pinv(np.dot(Ht, H) + identity/C[1])
    # print("*" * 50)
    ...
    test_h = TriT2_sigmoidrnd(weight, x_test, bias2, test_nums)
