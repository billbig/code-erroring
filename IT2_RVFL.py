...
def it2_sigmoidrnd(weight, train, bias):
...
def out_put(weight, train, bias):
    h = it2_sigmoidrnd(weight, train, bias)
    hp = np.concatenate((train, h), axis=1)
    ht = np.transpose(hp)
    return hp, ht


def IT2_RVFL(x_train, x_test, y_train, y_test):
    weight = all_weight()
    ...
    test_h, test_ht = out_put(weight, x_test, bias2)
